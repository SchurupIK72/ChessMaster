...
export default function ChessBoard({
  ...
  const topPlayer = flipped
    ? { label: "White Player", icon: "♔", markerClass: "w-4 h-4 bg-white border-2 border-black rounded-full" }
    : { label: "Black Player", icon: "♛", markerClass: "w-4 h-4 bg-black rounded-full" };
  ...
  const isFogActive =
    fogActiveOverride ??
    (Array.isArray(rules) && rules.includes("fog-of-war" as any) && (gameState.fullmoveNumber ?? 1) <= effectiveCutoff);

  const isSquareMasked = (rank: number): boolean => {
    if (!isFogActive || !viewerColor) return false;
    if (viewerColor === "white") return rank >= 5;
    if (viewerColor === "black") return rank <= 4;
    return false;
  };
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isBurned &&
            "after:absolute after:inset-0 after:bg-gradient-to-br after:from-slate-900/70 after:to-red-900/60 after:z-[12]",
        )}
        onClick={() => {
          if (!interactive || isBurned) return;
          if (isMasked) {
      ...
      >
        {isMasked && <div className="absolute inset-0 bg-slate-900/40 pointer-events-none z-[5]" />}

        {isBurned && (
          <div className="absolute inset-0 flex items-center justify-center z-[15] pointer-events-none">
            <span className="text-red-300/90 drop-shadow-lg select-none" style={{ fontSize: "1.75rem" }}>
              ✖
            </span>
          </div>
        )}
        ...
        {piece && !effectiveMasked && !isBurned && (
          <span
            className={cn("select-none font-bold relative z-[10]", piece.color === "white" ? "text-white" : "text-black")}
            style={{
              filter:
                piece.color === "white"
    ...
    <div className="bg-white rounded-[28px] shadow-[0_22px_60px_rgba(0,0,0,0.18)] border border-black/10 p-6 w-full max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <div className={topPlayer.markerClass} />
          <span className="font-medium text-neutral-900">{topPlayer.label}</span>
        </div>
        <div className="text-sm text-neutral-500">
          <span>{topPlayer.icon}</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-8 grid-rows-8 gap-0 w-full h-full relative">
          ...
          {ranks.map((rank, i) => (
            <span
              ...
              style={{ left: 2, top: `calc(${i * 12.5}% + 8px)`, zIndex: 20 }}
            >
              {rank}
            </span>
          ))}
          ...
          {files.map((file, i) => (
            <span
              key={file}
              className="absolute text-white/85 text-xs font-bold drop-shadow pointer-events-none select-none"
              style={{ left: `calc(${i * 12.5}% + 18px)`, bottom: 2, zIndex: 20 }}
            >
              {file}
            </span>
          ))}
      ...
      <div className="flex items-center justify-between mt-6">
        <div className="flex items-center space-x-3">
          <div className={bottomPlayer.markerClass} />
          <span className="font-medium text-neutral-900">{bottomPlayer.label}</span>
        </div>
        <div className="text-sm text-neutral-500">
          <span>{bottomPlayer.icon}</span>
        </div>
      </div>
