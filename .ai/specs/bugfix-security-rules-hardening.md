...
### Текущий статус
- Этап 4 завершён: logout стал server-authoritative, клиент корректно очищает auth/runtime state, duplicate `phone` маппится в business `400`, а global error handler больше не роняет процесс.
- Этап 5 пропущен по решению пользователя для ускорения релизной проверки на Render.
- Следующий шаг: подготовка к `merge-helper`, обновление документации и ручной smoke после деплоя.
...
**Задачи:**
- [x] Перевести logout на server-authoritative завершение сессии без ложного client-side восстановления.
- [x] Очистить client-side auth state после logout без повторного auto-login из localStorage.
- [x] Добавить duplicate `phone` check и корректный mapping DB unique violations в `400`.
- [x] Убрать повторный `throw` из global error handler.
...
| 2026-04-02 | 4 | 9cf1006 | Logout переведён на server-authoritative flow, duplicate phone возвращает business 400, а global error handler завершает ответ без повторного throw |
