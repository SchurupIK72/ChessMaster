...
### Текущий статус
- Этап 1 завершён: публичные request schemas отделены от `insert*Schema`, mutating game routes используют session-based ownership, `captured` закрыт как derived state.
- Следующий шаг: реализовать этап 2 через `spec-implementer`.
