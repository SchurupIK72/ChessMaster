...
### Текущий статус
- Спецификация подготовлена на основе `fixed spec.md` и backlog-анализа текущего `main`.
- Следующий шаг: реализовать этап 1 через `spec-implementer`.
...
**Задачи:**
- [ ] Создать public request schemas для auth, create game, join, move, draw и resign/status-like действий.
- [ ] Перевести `guest`, `create game`, `join game` и mutating game actions на обязательное использование `session.userId`.
- [ ] Добавить или дооформить server-side helper для определения роли пользователя в партии: `white`, `black`, `spectator`.
- [ ] Ограничить публичные маршруты, которые позволяют клиенту менять производное состояние напрямую.
...
**Задачи:**
- [ ] Усилить server-side проверку стандартного move flow: geometry, blocking, captures, en passant, castling, promotion, king safety.
- [ ] Перестать принимать от клиента `player`, `piece`, `captured`, `moveNumber`, `fen` как authoritative values.
- [ ] Нормализовать move record на сервере перед сохранением.
- [ ] Зафиксировать auto-complete сценарии завершения партии внутри server move flow.
...
**Задачи:**
- [ ] Проверить и выровнять `void`, `double-knight`, `fischer-random` между live route и `rebuildGameStateFromMoves`.
- [ ] Убедиться, что `undo` в сложных режимах восстанавливает то же состояние, что и live match.
- [ ] Удалить или жёстко ограничить `status` и `captured` endpoints.
- [ ] Перевести resign и terminal transitions на explicit server-owned flows.
